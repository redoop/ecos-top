...
# ============================================================================
# 合并的约束 - 来自 timing_constraints.sdc
# 修改日期: 2025-12-24
# 修改目的: 解决 RISC-V 内存接口的 untested 时序路径问题
# ============================================================================

# 内存接口时钟约束 - 针对 RISC-V 模块
create_clock -period 10.0 -name mem_clk [get_pins riscv/mem_clk] -waveform {0 5.0}
...
# 针对 untested 路径的假路径约束
# 修正日期: 2025-12-24
# 修正说明: 经RTL分析，G信号为组合逻辑(mem_wordsize控制)，不是时钟
# 这些锁存器的门控信号应设置为假路径，而非时钟约束
...
# 原有的假路径约束保留
set_false_path -from [get_pins riscv/mem_la_wdata_reg_*/G] -to [get_pins riscv/mem_la_wdata_reg_*/D]
set_false_path -from [get_pins riscv/mem_la_wstrb_reg_*/G] -to [get_pins riscv/mem_la_wstrb_reg_*/D]
set_false_path -from [get_pins riscv/mem_rdata_word_reg_*/G] -to [get_pins riscv/mem_rdata_word_reg_*/D]

# 额外的 I/O 延迟约束 - 内存接口
set_input_delay -clock sys_clk -max 2.0 [get_ports {io_addr io_wdata io_wen io_ren io_valid}]
set_input_delay -clock sys_clk -min 0.5 [get_ports {io_addr io_wdata io_wen io_ren io_valid}]
set_output_delay -clock sys_clk -max 2.0 [get_ports {io_rdata io_ready}]
set_output_delay -clock sys_clk -min 0.5 [get_ports {io_rdata io_ready}]

# SPI 接口额外约束
set_output_delay -clock spi_clk -max 1.0 [get_ports {io_spi_mosi io_spi_cs io_spi_dc}]
set_output_delay -clock spi_clk -min 0.2 [get_ports {io_spi_mosi io_spi_cs io_spi_dc}]
